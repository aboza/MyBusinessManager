...
public class DAOReceipt {
    ...
    public ArrayList<Receipt> getAllReceipts() {
        try {
            ...
            PreparedStatement statement = connection.getConnection().prepareStatement(Constants.ORACLE_GET_RECEIPT);
        ...
        try {
            ...
            while (var_resultSet.next()) {
                ...
                while (setResultados.next()) {
                    ReceiptDetail actualReceiptDetail;
                    actualReceiptDetail = new ReceiptDetail();
                    actualReceiptDetail.setItemID(var_resultSet.getInt("ITEM_ID"));
                    actualReceiptDetail.setQuantity(var_resultSet.getInt("QUANTITY"));
                    actualReceiptDetail.setTax(var_resultSet.getInt("TAX"));
                    actualReceiptDetail.setCost(var_resultSet.getInt("COST"));
                    actualReceiptDetail.setTotalAmount(var_resultSet.getInt("TOTAL_AMOUNT"));
                    actualReceiptDetail.setDiscount(var_resultSet.getInt("DISCOUNT"));
                    listaDetalles.add(actualReceiptDetail);
                }
